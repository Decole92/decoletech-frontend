...
export default function RegisterPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name) {
      ...
      newErrors.password = "Name must be at least 2 characters long";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          `Registration Successful!, ${result?.name} account been created.`
        );
  ...
  const handleGoogleSignUp = async () => {
    try {
      // Here you would integrate with your OAuth provider (NextAuth, Clerk, etc.)
          ...
          <Card className='shadow-2xl border-0 bg-white/95 backdrop-blur-sm'>
            <CardHeader className='text-center pb-8'>
              ...
              <CardDescription className='text-lg text-gray-600'>
                Join me to start your development journey
              </CardDescription>
            </CardHeader>
